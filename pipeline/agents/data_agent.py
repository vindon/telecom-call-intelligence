            ...
            if len(t["transcript_text"]) < MIN_TRANSCRIPT_CHARS:
                ...
                dl.log(
                    ...
                    evidence={
                        "call_id": str(call_id),
                        "chars": txt_len,
                        "min_required": MIN_TRANSCRIPT_CHARS,
                    },
                    call_id=str(call_id),
                    confidence="high",
                    alternatives=[
                        "Include with lower quality flag (rejected: extraction unreliable)"
                    ],
                )
                continue
            ...
            if t.get("turn_count", 0) < MIN_TURN_COUNT:
                ...
                dl.log(
                    ...
                    evidence={
                        "call_id": str(call_id),
                        "turns": turns,
                        "min_required": MIN_TURN_COUNT,
                    },
                    call_id=str(call_id),
                    confidence="high",
                    alternatives=[
                        "Include single-turn transcripts (rejected: not customer care calls)"
                    ],
                )
                continue
