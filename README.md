...
![Tests](https://img.shields.io/badge/Tests-481%20passing-22C55E)
...
| **481 unit tests** | Security, governance, decision log, memory, orchestrator, LLM clients, circuit breaker, config, graph, drift detection — all tested without API calls | CI completes in under 7 seconds; tests gate every push to main |
...
| Test suite | 481 tests · < 7 seconds |
...
```bash
make test          # 481 unit tests — no API calls required (< 7 seconds)
...
│
├── tests/                      ← 481 unit tests (zero API calls, < 7 seconds)
