...
![Tests](https://img.shields.io/badge/Tests-495%20passing-22C55E)
...
| **495 unit tests** | Security, governance, decision log, memory, orchestrator, LLM clients, circuit breaker, config, graph, drift detection, golden-set eval — all tested without API calls | CI completes in under 7 seconds; tests gate every push to main |
...
| Test suite | 495 tests · < 7 seconds |
...
```bash
make test          # 495 unit tests — no API calls required (< 7 seconds)
...
│
├── tests/                      ← 495 unit tests (zero API calls, < 7 seconds)
