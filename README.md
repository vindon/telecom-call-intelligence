...
![Tests](https://img.shields.io/badge/Tests-502%20passing-22C55E)
...
| **502 unit tests** | Security, governance, decision log, memory, orchestrator, LLM clients, circuit breaker, config, graph, drift detection, golden-set eval — all tested without API calls | CI completes in under 7 seconds; tests gate every push to main |
...
| Test suite | 502 tests · < 7 seconds |
...
```bash
make test          # 502 unit tests — no API calls required (< 7 seconds)
...
│
├── tests/                      ← 502 unit tests (zero API calls, < 7 seconds)
