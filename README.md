...
![Tests](https://img.shields.io/badge/Tests-503%20passing-22C55E)
...
| **503 unit tests** | Security, governance, decision log, memory, orchestrator, LLM clients, circuit breaker, config, graph, drift detection, golden-set eval — all tested without API calls | CI completes in under 7 seconds; tests gate every push to main |
...
| Test suite | 503 tests · < 7 seconds |
...
```bash
make test          # 503 unit tests — no API calls required (< 7 seconds)
...
│
├── tests/                      ← 503 unit tests (zero API calls, < 7 seconds)
...
│   └── engineering-standards.md ← Testing, CI gates, dependency pinning, token-economics/retry policy — what's enforced and where
...
- [`docs/engineering-standards.md`](docs/engineering-standards.md) — Testing, CI gates, dependency pinning, token-economics/retry policy, and known gaps
