...
![Tests](https://img.shields.io/badge/Tests-480%20passing-22C55E)
...
| **480 unit tests** | Security, governance, decision log, memory, orchestrator, LLM clients, circuit breaker, config, graph, drift detection — all tested without API calls | CI completes in under 7 seconds; tests gate every push to main |
...
| Test suite | 480 tests · < 7 seconds |
...
```bash
make test          # 480 unit tests — no API calls required (< 7 seconds)
...
│
├── tests/                      ← 480 unit tests (zero API calls, < 7 seconds)
